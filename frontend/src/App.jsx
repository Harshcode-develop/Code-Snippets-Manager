...
const SnippetsPage = lazy(() => import('./pages/SnippetsPage').then(module => ({ default: module.SnippetsPage })));
const StarredPage = lazy(() => import('./pages/StarredPage').then(module => ({ default: module.StarredPage })));
const FolderViewPage = lazy(() => import('./pages/FolderViewPage').then(module => ({ default: module.FolderViewPage })));

// Eager load critical UI components or keep them lazy if large
...
const SnippetViewModal = lazy(() => import('./components/SnippetViewModal').then(module => ({ default: module.SnippetViewModal })));
...
export default function App() {
    ...
    const handleUpdateSnippet = async (snippetId, updatedTitle, updatedCode) => {
        try {
            ...
            // Update local state if this snippet is currently open in AI Chat
            if (aiChatSnippet && aiChatSnippet.id === snippetId) {
                setAiChatSnippet(prev => ({ ...prev, title: updatedTitle, code: updatedCode }));
            }
    ...
    const handleAiChatQuery = async (userPrompt, code, history = []) => {
        // Format history for the prompt
        const historyText = history.map(msg => `${msg.sender === 'user' ? 'User' : 'AI'}: ${msg.text}`).join('\n');
...
System Instructions:
1. If the user asks about improving performance or optimizing the code:
   - Analyze the code for performance bottlenecks.
   - Briefly explain the potential improvements.
   - CRITICAL: DO NOT provide the full optimized code yet.
   - ASK the user: "Do you want me to change your existing code snippet for better performance?"
2. If the user confirms (says "yes", "sure", "do it", etc.) AFTER you asked about performance:
   - Provide the FULL, OPTIMIZED code block.
   - Ensure the code is complete and ready to replace the existing snippet.
3. For other questions, answer concisely and helpfully.
...
`;
    ...
    return (
        <div className="min-h-screen bg-vibrant-gradient text-text font-sans selection:bg-primary selection:text-white">
            {token && <Navbar onLogout={handleLogout} searchTerm={searchTerm} setSearchTerm={setSearchTerm} setPage={handlePageChange} page={page} onOpenApiKeyModal={() => setIsApiKeyModalOpen(true)} />}
              <main>
                <Suspense fallback={
